...
genre_song_counts = dt_lyrics %>% count(genre, sort = TRUE)
...
trigram_genre_year_data = trigram_over_time %>% 
  select(year, genre_popular_trigram_count_by_year, genre_w1_w2_w3, count) %>% 
  unique() %>% 
  filter(
...
ggplot(average_genre_sentiment_over_time, aes(x=year, y=average_genre_sentiment, col=genre)) + 
  ...
  xlab("Year") + 
  ggtitle("Each Genre's Average Sentiment over Time")
